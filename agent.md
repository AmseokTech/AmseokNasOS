...
- 已按用户明确要求完善 Web 应用商店的安全静态演示：侧边栏新增“应用尝鲜”入口，Studio Sync 尝鲜状态提升至应用商店页面，在切换订阅、下载等子页面后仍保留，且不安装应用；更新与下载中心新增进度条和图标动画，动画结束后先校验仓库内 `studio-sync-demo.txt` 的 HTTP 响应，成功才以 Blob 交给浏览器下载，失败会显示明确错误，不请求安装、账户、系统写入或后端新增接口。Photo Library、Studio Sync、Screen Cast 和 Backup Vault 均使用仓库内 512×512 的应用缩略图，并提供同窗口内的只读详情页、返回探索入口和明确的未开放安装提示；探索页卡片改为 270px 高的横向紧凑布局，图片区和列宽会随内容区在 160px 至 196px 间收缩，内容区宽度低于 740px 时才切换为单列，低于 540px 时图片移至文字下方并缩至 128px，因此中等宽度窗口仍可双列展示，真正过窄时不会挤压文字或覆盖内容。搜索框在探索、应用尝鲜、产品订阅和更新与下载页面均可见，输入关键词会回到探索并显示匹配结果。开发机 Angular ESLint 0 告警、17 个测试文件共 37 项测试和生产构建均通过；详情页不触发安装、订阅、权限申请或本地数据写入；受保护桌面端到端操作仍需使用当前数据库中的管理员密码登录
- 已在桌面顶栏新增仅前端的本地通知中心：铃铛入口位于时间与头像之间，支持未读角标、展开/收起、点击外部或 Escape 关闭、全部标为已读和逐条关闭；面板明确标注为本地示例，不读取、写入或伪造后端告警。通知面板现使用 Angular `animate.enter` / `animate.leave` 配合 CSS 实现右上角缩放淡入、反向收起和列表渐进上浮，并为减少动态效果偏好提供降级；Angular ESLint 0 告警、18 个测试文件共 40 项测试和生产构建均通过，通知样式保持在 4 KiB 单组件预算内
- 已在桌面顶栏日期时间入口增加仅前端的下拉日历：使用官方 Angular Material `MatCalendar` 和原生日期适配器，支持月份切换、日期选择、返回今天、点击外部或 Escape 关闭，并与通知中心互斥展开；面板沿用 Angular `animate.enter` / `animate.leave` 的右上角缩放淡入、反向收起和内容渐进上浮动画，并为减少动态效果偏好提供降级。日期格通过受限的原生日期适配器仅显示数字，不附加“日”，日期网格保留星期偏移但隐藏 Material 重复输出的月份标签。Angular ESLint 0 告警、20 个测试文件共 50 项测试和生产构建均通过，未新增组件样式预算告警；浏览器已加载最新开发前端，但受管理员登录状态阻挡，尚未完成人工登录后视觉回归
- 已在 Web 系统设置新增“语言与地区”，提供中文（简体）与 English 即时切换并以 `amseokos.language` 保存在当前浏览器；首次进入且尚未明确选择语言时固定使用中文，用户主动选择后才恢复持久化语言。语言服务、翻译管道和本地化运行时长均位于独立 `core/i18n` 边界，未增加后端、接口或数据库依赖。桌面顶栏、Dock、窗口控件、登录与改密页、关于本机、网络与磁盘设置、终端及应用商店已接入运行时文案，Angular Material 日历会同步区域设置，应用商店搜索会按当前语言匹配；语言设置页面已改为纵向选择列表、选中状态和即时界面预览，并按设置窗口自身宽度响应。Angular ESLint 0 告警、21 个测试文件共 61 项测试和生产构建通过，浏览器确认首次登录页显示中文且 `html lang=zh-CN` 生效；后端返回的自由文本错误仍由服务端原样展示
...
最新下一建议工作项（覆盖上方已部分完成的旧建议）：先在具备 root 权限的隔离 Debian 环境只使用明确创建的临时镜像和 loop 设备，逐级验证 RAID0/1/5/6/10 创建/删除、成员添加/移除/替换、RAID0/1/5/6 扩缩容、文件系统签名缩容拒绝、系统盘/挂载/holder/swap 拒绝、命令超时与 daemon/API 重启后的幂等和 Operation 复核；通过后再重建签名 Debian 包、部署测试节点并完成管理员登录态浏览器回归。并行补签名发布过程的原子索引切换、旧包保留策略和全新 Debian 13 节点一次性安装验证，为测试 APT 仓库配置 HTTPS、离线加密备份签名私钥和演练轮换；再推进 Data Protection 外部密钥保护、会话跨重启、etcd Leader/fencing、NATS Inbox/Outbox 和网络安全回滚。真实 loop/mdadm 门禁未通过前不得把本次 RAID 写入发布为生产可用
