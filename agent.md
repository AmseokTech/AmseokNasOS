...
- 当前功能代码基线提交为 `887bb57 build(debian): 补齐存储与共享运行依赖`
...
最新下一建议工作项（覆盖上方已部分完成的旧建议）：先由管理员登录 `.7` 的 `https://192.168.188.7:6521/`，核对数据卷预览显示的目标确为未格式化 `/dev/md127`，仅在明确接受清空该真实阵列时完成两阶段初始化，再从独立局域网客户端验证 SMB/NFS 实际读写、权限和整机重启恢复；在获得该破坏性操作授权前不得格式化 `.7`。随后补齐 RAID0/1/5/6/10 全生命周期的隔离 Linux 矩阵、命令超时与 daemon/API 重启后的幂等和 Operation 复核，补签名发布过程的原子索引切换、旧包保留策略和全新 Debian 13 节点一次性安装验证，并为测试 APT 仓库配置 HTTPS、离线加密备份签名私钥和演练轮换；再推进 Data Protection 外部密钥保护、会话跨重启、etcd Leader/fencing、NATS Inbox/Outbox 和网络安全回滚
