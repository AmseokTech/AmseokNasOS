...
- 密码修改后旧密码失效的部署验证、真实认证 Cookie 跨重启持久化验证、PostgreSQL HA、etcd Leader/fencing 应用接入、NATS Inbox/Outbox 工作者、登录态浏览器中的系统/网络/磁盘/RAID/Dashboard 查询与 Terminal WebSocket 人工回归、网络配置执行与自动回滚仍未完成，第一阶段验收条件尚未满足
...
最新下一建议工作项（覆盖上方已部分完成的旧建议）：第一阶段继续补密码修改后旧密码失效与真实认证 Cookie 跨服务重启验证，再接入 etcd Leader/fencing、NATS Inbox/Outbox 工作者和网络配置安全写入/连通性确认/超时回滚，并完成登录态浏览器中的系统、网络、磁盘、RAID、SMART、Dashboard 与 Terminal WebSocket 人工回归。`.7` 的 `/dev/md127` 仍必须保持未格式化、未挂载，只有管理员明确接受清空真实阵列并完成 Web 两阶段确认后，才能执行初始化及后续 SMB/NFS 独立客户端读写和整机重启恢复验证。其后按阶段顺序完成 RAID 全生命周期故障矩阵、告警/备份/恢复、配置漂移、应用中心安全模型、安装器真实执行器、live-build ISO/QEMU、APT 原子发布/HTTPS/版本单调递增检查/Data Protection 证书离线备份与轮换；任何未通过对应验收门禁的功能不得标记为完成
