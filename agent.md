...
当前部署构建说明：磁盘 SMART 摘要已经完成前端、API、Application 端口和 Rust 特权执行面的只读纵切并部署验证；`.7` 当前运行测试版本 `0.1.0+test20260810.c8ea3b7.smart4`。该版本由 `.10` 的 `main` 工作树叠加本地尚未提交的存储、SMART、Dashboard 与 Data Protection 改动后构建，不是纯提交构建；正式发布前必须先在本地解决 `devkihon` 与 `origin/main` 分叉并按授权完成审查、提交和推送
...
部署状态更新（2026-08-20）：最新 `origin/main` 提交 `848827b9cec289301260fad3d1b751fa5a17765b` 在 `.10` 的仓库打包门禁中暴露桌面应用判别联合、SVG 图标契约和测试夹具未同步的问题；最小修复已在本地完成 Angular ESLint、27 个测试文件共 72 项覆盖率测试、生产构建和生产依赖 0 漏洞验证，以 `f3f93409e86a629f0bee2aed00f4d53a10283553` 推送到 `main`。因 `.10` 无法直连 GitHub，修复后的 `origin/main` 通过完整 Git bundle 传输并由 SHA-256 校验后，在干净 detached 工作树中以 fast-forward fetch 更新，未处理或覆盖其他远端工作树。版本 `0.1.0+test20260820.zzzf3f9340.main1` 的七个 Debian 包完成 Angular 72 项覆盖率测试与生产构建、.NET 87 项测试和 self-contained 发布、privileged 61 项与 Terminal 4 项 Rust 格式/Clippy/测试/Release、Console 测试、逐包 Debian 元数据、生产依赖 0 漏洞和 SHA-256 验证；第一次前端整套测试因构建机瞬时负载出现 5 个超时，未跳过或放宽测试，原命令随后单独复跑和权威打包脚本复跑均 72/72 通过。`.10` 已使用既有 Ed25519 仓库密钥重新签名 `InRelease` 和 `SHA256SUMS.asc`，指纹 `313FE99FB604BC0F7DF0EE5C2DF4F37A9B11EEDB` 验证通过；`.7` 已 curl 七包共 42,917,472 字节和安装器，验证签名清单、全部 SHA-256、包版本与元数据后升级。七包安装版本和 APT Candidate 一致，`dpkg -V`、`dpkg --audit`、`apt-get check` 均通过；PostgreSQL 实例、etcd、NATS、privileged、Terminal、API、Nginx 与 Console 均 active/enabled，主进程状态 0、重启次数 0，实际 PostgreSQL 实例和其他长期服务具备重启策略，etcd、NATS、API live/ready 与 HTTPS 前端健康，失败 unit 和部署后十分钟 warning 级日志均为 0，两个 Unix Socket 为服务专用账户和组所有的 `0660`。真实 `/dev/md127` 仍为健康 `[UU]` RAID1，没有文件系统类型或挂载点

前端视觉更新（2026-08-20）：桌面窗口普通视窗圆角由 `14px` 增大到 `28px`，窄屏视窗由 `10px` 增大到 `24px`，最大化窗口继续保持直角；共享 `WindowFrame` 的三个 `13px` 控制圆点位于右上角，并按从右向左关闭（红）、放大/还原（绿）、最小化（黄）的顺序排列，标题栏移除灰色背景和下边线并改为完全透明，窗口宿主按概览、设置、应用商店和终端匹配内容表面色与标题对比度，保留原窗口事件、拖拽热区、动态可访问名称、键盘焦点反馈与减少动画适配。Angular ESLint、窗口框架/窗口宿主 2 个测试文件共 4 项测试和生产构建通过；本地开发页面确认概览与设置窗口标题栏计算背景均为 `rgba(0, 0, 0, 0)`、下边线为 `0px`，对应窗口表面色分别为 `rgb(244, 247, 251)` 与 `rgb(242, 243, 245)`，控制圆点的实际横向顺序为黄、绿、红且最右侧关闭按钮及放大/还原交互保持正常

前端图标更新（2026-08-20）：Dock 与启动台中的概览、终端、应用商店、系统设置和启动台已改用用户提供的完整 SVG 图标资源；资源继续由本地静态目录提供，应用展示契约与启动逻辑保持不变，Dock 不再为这五个完整图标重复叠加旧渐变底板，并移除 SVG 内置 `feDropShadow` 以消除图标周围的黑色光晕；Dock 同时移除会被横向滚动容器裁切的可视悬停提示层，按钮继续以 `aria-label` 提供可访问名称，消除图标上方露出的深色残片。五项资源通过 XML 校验，Angular ESLint、Dock/启动台/桌面 3 个测试文件共 10 项测试和生产构建通过；悬停提示修复后 Dock/桌面 2 个测试文件共 8 项测试、Angular ESLint 和生产构建再次通过，本地开发页面确认 Dock 加载 5 项、启动台加载 4 项新资源、可视提示层数量为 0，并在重新选中概览后保持 selected、running 和 focused 状态

启动台视觉更新（2026-08-20）：启动台覆盖起点由 `40px` 调整为与顶部系统栏实际底边一致的 `34px`，消除两者之间固定露出的 `6px` 壁纸缺口；全屏深色遮罩改为白色半透明渐变与 `36px` 模糊的毛玻璃背景，移除可见的 `AMSEOKOS`、`启动台` 和已安装组件数量标题区，Dialog 改用直接可访问名称；应用网格取消居中与弹性拉伸，改为从左上角使用 `104px` 至 `128px` 固定范围轨道自然换行，同时保留关闭按钮、Esc 退出、焦点约束和应用启动事件。Angular ESLint、启动台/桌面 2 个测试文件共 8 项测试和生产构建通过；本地 `395×933` 开发页面确认顶栏与启动台间隙为 `0`、可见文本仅含 4 个应用名称、首个应用位于左侧 `18px` 且内容区顶部 `22px`，背景计算值为 `rgba(255, 255, 255, 0.68)` 并启用 `blur(36px) saturate(1.45)`

终端认证窗口更新（2026-08-20）：终端管理员重新认证已从 Angular Material Dialog 迁入内部单例 `terminal-auth` 受管窗口，复用共享 `WindowFrame`，支持拖动、最小化与 Dock 恢复、最大化/还原、关闭和布局持久化；认证成功后才关闭认证窗口，并将一次性会话作为仅运行时数据交给正式 Terminal 窗口，窗口销毁时会取消未完成的认证订阅，避免迟到响应意外打开终端。认证窗口及标题栏明确使用 `PingFang SC` 与系统中文无衬线回退，xterm 命令区继续保留等宽字体以维持字符网格。Angular ESLint、5 个测试文件共 16 项测试和生产构建通过；本地开发页面确认认证界面拥有共享窗口框架的最小化、最大化和关闭控件，不再存在 Material Dialog，计算字体为 `PingFang SC`，最小化后通过 Dock 恢复以及最大化/还原交互均正常

系统概览视觉更新（2026-08-20）：概览内容区已移除可见的 `SYSTEM OVERVIEW` 英文眉题、重复的“系统概览”主标题及只读状态说明，更新时间与刷新操作改为独立右对齐工具栏，状态卡片和数据聚合逻辑保持不变；页面继续以不可见的 `aria-label="系统概览"` 保留区域语义。Dashboard 组件测试、Angular ESLint 和生产构建通过；本地开发页面确认三段目标文字均不可见、刷新操作存在且 7 张状态卡片正常渲染

系统概览性能视图更新（2026-08-20）：概览已改为 Windows 任务管理器式性能布局，左侧按 CPU、内存、物理磁盘、物理网络接口与 GPU 列出资源，右侧提供 CPU 每逻辑处理器独立 60 秒曲线、实时占用与频率、物理/逻辑核心数和 L1-L3 缓存，以及内存、磁盘吞吐/活动时间、网络收发和 GPU 核心/2D/3D/显存/频率详情；驱动不暴露的 GPU 引擎或显存指标明确显示“不支持”，不推测数据。privileged 新增 `system.inspectPerformance` 只读动作，从 `/proc` 与 `/sys` 返回原始累计计数器，API 以 `system.read` 暴露 `/api/system/performance`，Angular 以 1 秒间隔计算相邻快照差值并保留 30 秒原概览聚合作为旧服务和开发环境的静态降级。Rust 64 项测试及全目标 Clippy `-D warnings`、.NET 解决方案零警告构建和相关 16 项契约测试、Angular 28 个测试文件共 77 项测试、ESLint 与生产构建通过；本地开发页面在窄屏和 `1280×900` 宽屏确认资源列表、8 个逻辑处理器图表、详情指标和内存切换正常，开发 mock 尚未提供实时端点，因此本地预览按设计显示静态降级提示，尚未在 Linux 真机验证真实 GPU 驱动计数器

Amtest WSL 部署更新（2026-08-21）：Windows Server 2022 测试机 `192.168.2.213` 已在 `C:\Users\Administrator\Amtest` 安装 WSL `2.7.12.0`，以 `C:\Users\Administrator\Amtest\wsl\ext4.vhdx` 运行 Debian 13.5 trixie；本地当前 `f3f93409e86a629f0bee2aed00f4d53a10283553` 工作树连同未提交的性能视图、窗口、启动台、图标与终端改动通过 SHA-256 为 `b8fd11dafe016b3d81a65288d343605e3e3c1fbd59412d26cf923cbeaf4855a0` 的源码归档传入，目标机源码不含 `.git`，本次仍是工作树测试部署而不是纯提交构建。目标 WSL 使用 Node.js `22.22.3`/npm `10.9.8`、.NET SDK `10.0.302` 和 Rust/Cargo/Clippy `1.97.1` 构建版本 `0.1.0+test20260821.amtest1` 的七个 Debian 包，Angular ESLint、28 个测试文件共 77 项覆盖率测试和生产构建、.NET 89 项测试与 linux-x64 self-contained 发布、privileged 64 项与 Terminal 4 项 Rust 格式/Clippy/测试/musl Release、Console 测试及逐包 SHA-256 均通过。七包安装后 PostgreSQL 17 实例、etcd、NATS、privileged、Terminal、API 与 Nginx 均 active/enabled，主进程状态 0、重启次数 0，WSL 无物理 tty1，因此 Console 包已安装但 service 明确 disabled/inactive；`dpkg -V`、`dpkg --audit`、`apt-get check`、失败 unit、API 错误日志与除安装期一次证书生成前 Nginx reload 以外的服务 warning 复核通过，两个 Unix Socket 为专用账户/组所有的 `0660`，自动迁移已关闭。Windows 已建立不保存明文密码的 `Amtest WSL Runtime` 开机 S4U 任务，持续保活发行版并刷新 `0.0.0.0:6521` 到当前 WSL 动态地址的 portproxy，同时只新增 TCP 6521 入站防火墙规则；WSL 内部、Windows 本机及局域网客户端对前端、live 和 ready 的检查均返回 200，未登录性能接口返回 401。通过 API 服务账户直连 privileged Socket 的真实 `system.inspectPerformance` 返回 i5-12400F、12 个逻辑处理器、16,679,317,504 字节可用内存视图、4 个块设备、1 个网络接口和空 GPU 列表。宿主 PCI `10DE:104A` 为 Fermi 架构 GeForce GT 610，虽然 WSL 已提供 `/dev/dxg`、Direct3D/DirectML/DXCore 库，但没有 `libcuda.so.1` 或 `nvidia-smi`，不满足 NVIDIA WSL GPU 对 Pascal 及以上架构的支持条件，不能通过安装驱动把该卡作为受支持的 WSL 计算 GPU；部署按 CPU 模式运行并由界面如实显示 GPU 不支持。局域网自动浏览器检查因自签名证书触发 `ERR_CERT_AUTHORITY_INVALID` 而按安全边界未绕过，页面 HTTP 内容、TLS 端点与 API 运行态已由 curl 和服务级检查验证

桌面控制中心更新（2026-08-24）：基于最新 `main` 提交 `9b9f55b` 的新版登录与桌面体验，在顶栏控制中心入口接入可交互的 macOS 27 风格面板，包含 Wi-Fi、蓝牙、隔空投送、专注模式、台前调度、屏幕镜像、显示亮度、深色模式、夜览、音量、电池、低电量模式和编辑控制；控制中心复用原 Wi-Fi 状态并与 Wi-Fi、通知和日历面板互斥，深浅色选择持久化后统一作用于四类系统面板，原通知、Wi-Fi 和日历业务行为保持不变。Angular ESLint、28 个测试文件共 80 项测试和生产构建通过；本地 `6521` 开发服务已确认由最新版工作树启动，首页和懒加载资源均返回 200，并同时包含新版登录与控制中心代码

检查结果：

- 已初始化本地 Git 仓库，本地 `devkihon` 基于并跟踪远端 `origin/devkihon`
- 已配置远端 `origin` 为 `git@github.com:AmseokTech/AmseokOS.git`
- 当前本地 `devkihon` 提交基线为 `c8ea3b75b97e0c5236a99e35b35b83c084ab1ae5 docs(agent): 记录数据卷闭环验证`，远端 `origin/main` 为 `c78e23ddf7e813b73b178560777cbda6d063ffce feat(storage): 实现数据卷底层操作`；两者已有分叉，当前 SMART 纵切及相关测试修复仍是本地未提交改动，不得把测试部署包版本误当作可追溯 Git 提交
...
- 已完成 RAID 生命周期代码闭环：Application 层新增独立 `IRaidManagementService`/`IRaidCommandExecutor` 边界，Controller 只负责 `raid.manage` 授权、CSRF、限速和 HTTP 映射；创建、删除、添加、移除、替换、扩容和缩容均先重新认证并生成两分钟一次性预览令牌，执行时再次认证、固定时间比较确认短语、重读库存指纹、原子创建持久化 Operation 和多资源锁、递增 fencing token，并写入审计 Outbox 意图。Rust privileged daemon 只登记七个固定 RAID 动作，拒绝未知字段，不经过 shell，使用固定 `mdadm`/`blkid` 路径和有界超时，执行前再次按稳定物理盘 ID 核对成员、系统盘、挂载、swap、holders、只读和占用状态；RAID0/1/5/6/10 支持创建，非 RAID0 支持成员管理，RAID0/1/5/6 支持成员数扩缩容，RAID10 成员数调整失败关闭。缩容只允许未挂载、无 holder/swap 且没有文件系统或其他数据签名的原始阵列，并强制使用 root 私有 reshape backup-file；命令开始后的任何不确定错误保留资源锁，后续 Operation 查询会按真实阵列成员、目标成员数和同步状态复核完成/失败。daemon 另有原子持久化幂等/fencing 登记，重复已完成请求返回原结果，执行中重复请求要求复核
- 系统设置“磁盘管理”现已启用完整两阶段管理交互：空闲安全整盘才会进入候选列表，支持 RAID0/1/5/6/10 创建、删除、成员添加/移除/替换以及受支持级别扩缩容；界面展示后端阻断原因和风险警告，要求再次输入密码与确认短语，并轮询后台 Operation 及断线复核状态。验证结果为 .NET 10 Release 66 项 xUnit 全部通过，privileged daemon 本机 50 项 Rust 测试和 macOS/Linux 目标 Clippy `-D warnings` 通过；GitHub CI #109 暴露新增 RAID 交互分支缺少测试后，已补磁盘候选安全过滤、阵列状态/进度/动作/Operation 文案、成员稳定身份映射和输入变更失效旧预览测试，Angular ESLint 0 告警、19 个测试文件共 43 项测试通过，语句/分支/函数/行覆盖率分别为 67.23%/50.21%/67.77%/66.56%，重新高于 60%/45%/60%/60% 门槛，生产构建通过并保留应用商店及磁盘管理组件样式预算告警。因 `.7/.10` 当前 SSH 专用凭据不可用，尚未执行隔离 Linux 真实 loop/mdadm 生命周期验证、Debian 包重建、部署和浏览器人工回归，不能把这些环境验证标记为完成
- 已新增独立 `StorageManagement` 纵向模块并实现六项数据卷能力：只允许健康、空闲、非降级且身份稳定的 MD 阵列进入初始化预览；Rust privileged daemon 以固定程序路径和结构化参数执行 `mkfs.ext4`，读取并保存文件系统 UUID，生成 `What=UUID=...` 的 systemd mount unit 并启用开机恢复；数据目录支持受限 owner/group/mode，读写校验使用独占临时文件、`fsync`、逐字节回读与清理；SMB 使用 `/etc/samba/smb.conf.d` 精确 include，NFS 使用 `/etc/exports.d`、网段约束和 `root_squash`，二者均先校验再加载。C# Application 负责双密码重新认证、两阶段预览、一次性令牌、确认短语、库存指纹、持久化 Operation/资源锁/fencing/Outbox 审计和断线复核，Angular 数据卷页面提供初始化、权限、共享和读写验证交互；未知字段、任意命令、任意路径及 IPv4 `/0` 网络范围均拒绝
- `.10` 已从 GitHub `main` 拉取最新提交 `9d76b8bb2922df36312952274233609ceff45a90` 后覆盖本地未提交的存储改动，全量验证 Angular 22 个测试文件共 51 项测试、生产构建、.NET 73 项 xUnit、privileged daemon 55 项 Rust 测试、Terminal 4 项 Rust 测试、格式化、Clippy 和 Release 构建通过。使用两个明确创建的 256MiB loop 文件组成隔离 RAID1 后，已真实验证 ext4 格式化、UUID 挂载、停止后重新启动 mount unit 恢复、0770/0775 权限切换、写入/回读、SMB `testparm` 可见共享及 NFS `exportfs -v` 的限定网段导出；测试共享、mount unit、描述文件、阵列、loop 与镜像均已清理，未接触 `.7` 的真实数据盘
- `.10` 已签名发布版本 `0.1.0+git20260809.zz9d76b8b.storage1` 的七个 Debian 包，`.7` 通过 `http://192.168.188.10/apt` curl、SHA-256 与签名仓库安装成功；七包版本一致，privileged/API/Terminal/etcd/NATS/Nginx 均 active/enabled，HTTPS 前端返回 200，未登录存储接口返回预期 401，失败 unit 与近期 warning journal 均为 0。`.7` 的 `/dev/md127` 仍由两块 20GB 设备组成健康 `[UU]` RAID1，当前没有文件系统 UUID、没有挂载点且未执行初始化；后续只有管理员在 Web 两阶段确认后才会格式化
- 已完成磁盘 SMART 只读纵切：Angular 只对稳定且无身份冲突的磁盘请求摘要并提供逐盘加载、错误和重试状态；C# 使用独立 Application 端口及受认证的 `GET /api/storage/disks/smart?deviceId=...`；Rust daemon 只接受稳定设备 ID，执行前重新枚举并解析当前设备路径，以固定 `smartctl` 路径、参数数组、清空继承环境、5 秒超时和 512 KiB 输出上限读取窄字段，区分健康、警告、故障、不支持和未知状态，拒绝客户端路径与未知字段。验证结果为本地 Rust 60 项测试和 Clippy `-D warnings` 通过；`.10` 使用工作树覆盖方式构建并签名发布测试版本 `0.1.0+test20260810.c8ea3b7.smart2` 的七包，Angular ESLint、22 个测试文件共 51 项测试、生产构建、.NET 75 项 xUnit、Rust 60+4 项测试和 Release 构建均通过；`.7` 从 `.10` 签名仓库 curl 升级后七项服务 active/enabled，健康接口和 Web 均返回 200，失败 unit 与最近十分钟 warning journal 均为 0。两块 VMware 20GB RAID 成员通过真实 Unix Socket 协议查询均成功返回 `unsupported`，与设备本身不提供 SMART 一致；健康 RAID1 仍为 `[UU]` 且未格式化、未挂载。该测试版本基于未提交覆盖内容，不是纯 Git 提交构建，不能作为正式发布基线
- 已完成 Dashboard 第一阶段只读纵切：概览作为桌面单例窗口通过功能公开入口聚合控制面健康、主机、CPU/内存、网络、系统盘、磁盘、RAID、SMART 与数据卷状态；各数据源独立保留错误，部分服务断开不会抹掉其他卡片，并提供手动刷新和 30 秒自动重试恢复。`.10` 的 Node 24.15 验证 Angular ESLint、24 个测试文件共 54 项测试和生产构建通过，语句/分支/函数/行覆盖率为 67.58%/50.76%/66.75%/66.95%；只保留原有三个样式预算告警。首次测试版本后缀 `dashboard1` 在 Debian 版本排序中低于已安装的 `smart2`，APT 正确拒绝降级；未绕过校验，改用单调递增的 `smart3` 重构建七包并重新签名发布。`.7` 通过 `.10` curl 安装 `smart3` 成功，七项服务 active/enabled、健康接口与 Web 均返回 200、失败 unit 和近期 warning 为 0，已安装懒加载制品包含 `SYSTEM OVERVIEW`；真实 RAID1 保持 `[UU]` 且无文件系统、无挂载。发布脚本仍需加入 Debian 版本单调递增的失败关闭检查
- 已完成 Data Protection 外部密钥保护：API 使用固定 Application Name，将密钥环持久化到 PostgreSQL，并以数据库之外的 RSA 证书加密每条密钥；缺少、不可读、私钥不匹配或证书过期均启动失败关闭。安装器仅在材料完全不存在时生成 3072 位 RSA 证书，部分存在则拒绝覆盖；目录为 `root:amseoknas-api 0750`，私钥为 `root:amseoknas-api 0640`，普通测试账户不可读。新增证书加载与缺失材料测试后 `.10` 的 .NET 77 项 xUnit 通过，并构建签名发布七包测试版本 `0.1.0+test20260810.c8ea3b7.smart4`；`.7` curl 安装后 PostgreSQL `DataProtectionKeys` 仅有一条记录且 XML 包含 `encryptedSecret`，重启 API 后，重启前签发的防伪令牌仍可通过校验进入密码模型验证，证明密钥环跨进程恢复有效。真实认证 Cookie 跨重启及修改密码后旧密码失效仍待使用有效 Web 管理员凭据验证；证书离线备份与轮换演练仍未完成
- 密码修改后旧密码失效的部署验证、真实认证 Cookie 跨重启持久化验证、PostgreSQL HA、etcd Leader/fencing 应用接入、NATS Inbox/Outbox 工作者、登录态浏览器中的系统/网络/磁盘/RAID/Dashboard 查询与 Terminal WebSocket 人工回归，以及网络配置在真实 Debian 物理/虚拟网卡上的 DHCP、静态地址、失联超时、daemon 与整机重启恢复 E2E 仍未完成，第一阶段验收条件尚未满足
...
最新下一建议工作项（覆盖上方已部分完成的旧建议）：第一阶段先在具备控制台或第二管理链路的 Debian 测试机完成网络配置 DHCP、静态地址、错误网关、未确认超时、daemon 与整机重启恢复 E2E，再补密码修改后旧密码失效与真实认证 Cookie 跨服务重启验证，接入 etcd Leader/fencing、NATS Inbox/Outbox 工作者，并完成登录态浏览器中的系统、网络、磁盘、RAID、SMART、Dashboard 与 Terminal WebSocket 人工回归。`.7` 的 `/dev/md127` 仍必须保持未格式化、未挂载，只有管理员明确接受清空真实阵列并完成 Web 两阶段确认后，才能执行初始化及后续 SMB/NFS 独立客户端读写和整机重启恢复验证。其后按阶段顺序完成 RAID 全生命周期故障矩阵、告警/备份/恢复、配置漂移、应用中心安全模型、安装器真实执行器、live-build ISO/QEMU、APT 原子发布/HTTPS/版本单调递增检查/Data Protection 证书离线备份与轮换；任何未通过对应验收门禁的功能不得标记为完成
