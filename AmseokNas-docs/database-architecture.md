...
AmseokNas 采用对等 NAS 节点和动态控制面 Leader。所有 NAS 安装相同程序，可以作为存储节点，也可以在满足资格和仲裁条件后成为控制面节点。
...
账户和权限使用 ASP.NET Core Identity 建立在 PostgreSQL 上。Identity 负责用户生命周期、密码哈希验证、安全戳、锁定和令牌基础能力；业务权限仍由 AmseokNas 的明确权限点控制。
